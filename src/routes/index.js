...
router.post('/auth/register',   isAdmin,
...
router.get ('/devices/:id',     param('id').isInt({ min: 1 }), validate, C.getDevice);

router.post('/devices',        isOfficer,
  ...
  body('affiliationId').isInt({ min: 1 }).withMessage('Affiliation is required'),
  validate, C.createDevice);

router.post('/devices/import', isOfficer, upload.single('file'), C.importDevices);

router.patch('/devices/:id',   isOfficer,
  param('id').isInt({ min: 1 }), validate, C.updateDevice);

router.post('/devices/:id/transfer', isAdmin,
  param('id').isInt({ min: 1 }),
  body('toFacilityId').isInt({ min: 1 }).withMessage('Target facility required'),
  validate, C.transferDevice);

router.delete('/devices/:id', isAdmin,
  param('id').isInt({ min: 1 }), validate, C.deleteDevice);

// ── Verifications
router.get ('/verifications',           C.listVerifications);
router.post('/devices/:id/verify', isOfficer,
  param('id').isInt({ min: 1 }),
  body('devicePresent').isBoolean(),
  body('overallStatus').isIn(['pass', 'fail', 'partial']),
  validate, C.verifyDevice);
